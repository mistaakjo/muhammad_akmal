...
DELTA                       =      -0.001              #   How much should the epsilon shift by in every game
...
STARTING_EPSILON            =      1.2

#These are the reward parameters. You need to change this. Refer to the guide to know what each means in detail
DESTROY_SOFTBLOCK           =      -50      
DO_DAMAGE                   =      1000
TAKE_DAMAGE                 =      -1000     
EARN_AMMO                   =      500
COLLIDE_WITH_WALLS          =      -10000
DAMAGE_ITSELF               =      -2500        
NO_DESTRUCTION              =      0                 
NO_KILL                     =      0             
PLACING_BOMBS_WITH_NO_AMMO  =      -10000
BOMB_IN_RANGE               =      -800
PLACING_BOMB_ON_TOP_OF_BOMB =      -10000
EXIST_PENALTY               =      -500
...
GUIDANCE                    =       0.85                         # 1 means the agent copies exactly as the coach. 0 means that the agent makes random moves Should be between 1 and 0
...
class Agent:
    ...
    def next_move(self, game_state, player_state):
        ...
        if(done):
            ...
            if(player_state.hp==0):     print("Alpha: LOST")
            else:                       print("Alpha: WIN")
